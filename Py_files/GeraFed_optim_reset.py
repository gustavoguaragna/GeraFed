...
# train_partitions = [train_partition.select(range(n)) for train_partition, n in zip(train_partitions, num_samples)] #### reduce sample size
...
checkpoint_loaded = torch.load("checkpoint_epoch68.pth", map_location=torch.device('cpu') )
...
for num_chunks in [100]:
    ...
    epoch_bar = tqdm(range(68, epochs), desc="Treinamento", leave=True, position=0)
    ...
    folder = f"Exp_Adam_reiniciando_gerafed_mnist"
