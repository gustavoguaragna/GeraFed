...
class F2U_GAN_CIFAR(nn.Module):
    def __init__(self, img_size=32, latent_dim=128, condition=True, seed=42):
        if seed is not None:
          torch.manual_seed(seed)
...
def syn_input(num_samples: int, 
    ...
    if round % num_chunks in (0,1,20):
        ...
        # Escolhe até 3 índices aleatórios dentro do tamanho do dataset
        num_to_save = min(3, len(generated_dataset))
